...
Claude supplied and replaced with the authenticated session's real id, before
any handler (and therefore any service/integration) ever sees it. This holds
even under prompt injection — e.g. a malicious ticket description telling
Claude to emit a different astrologer_id renders inert here, because the
value is never read, only overwritten.
...
def execute(tool_name: str, tool_input: dict, ctx: SessionContext) -> ToolResult:
    ...
    if spec is None:
        return ToolResult(
            content_for_claude=f"error: unknown tool '{tool_name}'",
    ...
    try:
        ...
        return ToolResult(
            content_for_claude="error: tool execution failed",
