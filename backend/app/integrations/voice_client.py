...
def _real_create_call(phone_number: str, call_id: int) -> OutboundCallResult:
    client = Client(settings.TWILIO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN)
    query = urlencode({"call_id": call_id, "secret": settings.TWILIO_WEBHOOK_SECRET})
...
def create_call(phone_number: str, call_id: int) -> OutboundCallResult:
    if not settings.VOICE_MOCK_MODE:
        return _real_create_call(phone_number, call_id)
